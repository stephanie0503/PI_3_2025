<template>
    <div ref="invoiceContent" class="px-4 py-2 bg-white">
      <!-- Cabeçalho -->
      <div class="flex justify-between mb-6">
        <div>
          <div class="flex items-center">
            <img class="w-[240px]" src="@/assets/images/dinamica-logo.png" alt="" />
          </div>
        </div>
        <div class="text-right">
          <h3 class="text-primary ml-2 font-bold">Dinâmica Informática e Telecomunicações Ltda</h3>
          <div class="mt-4">
      ...
      <div class="flex flex-col gap-y-[20px]">
        <div>
          ...
          <div class="bg-[#dbd7d7] text-black font-bold p-1 flex justify-between items-center">
            <h6>Dados para Faturamento e Entrega:</h6>
            <span>Data: {{ formatDate(data.created_at) }}</span>
          </div>
        </div>
            ...
            <el-col :span="18">
              <div class="grid-content">
                <div class="flex gap-x-[10px] items-center">
                  ...
                  <p>{{ data.client.address + ", " + data.client.number }}</p>
                </div>
              </div>
            </el-col>
            <el-col :span="6">
        ...
        <div class="overflow-x-auto">
          ...
          <table class="min-w-full text-sm border-[1px]">
            ...
            <tbody>
              <tr v-for="item in data.items" :key="item.id" class="border-b">
        ...
        <div class="mt-3">
          <h6 class="text-center bg-[#b5b1b1] text-black font-bold py-1">
            Aceite da Proposta Num: {{ data.proposal_number }}
          </h6>
          <div class="signatures h-[120px] flex flex-col justify-end">
            ...
            <div class="flex justify-between gap-x-2 items-center">
              <div class="flex w-[50%] justify-center text-center">
                <span class="text-black font-bold">{{ data.client.name }}</span>
              </div>
              <div class="flex w-[50%] justify-center text-center">
                <span class="text-black font-bold">DINÂMICA INFORM E TELECOMUNICAÇÕES LTDA</span>
              </div>
            </div>
          </div>
        </div>
  ...
  const subtotal = computed(() => {
    return props.data.items.reduce((acc, item) => acc + item.unit_price * item.quantity, 0);
  });
  
  // Calculando o total (usando desconto e taxa, por exemplo)
  ...
  const generatePDF = async () => {
    ...
    setTimeout(() => {
      doc.html(content, {
        ...
        x: margin,
        y: margin,
        autoPaging: true, // Isso ajuda a dividir o conteúdo em várias páginas automaticamente
        html2canvas: {
